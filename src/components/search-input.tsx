...
export function SearchInput({ onSearch, isDialog = false }: SearchInputProps) {
  ...
  const inputClassName = isDialog 
    ? "w-full h-12 px-4 pl-10 pr-12 bg-black/30 text-white placeholder:text-gray-300 border-white/20 focus:bg-black/40 focus:ring-primary text-base backdrop-blur-sm"
  ...
  return (
    <form onSubmit={handleSearchSubmit} className="relative w-full max-w-xl" ref={containerRef}>
      {isDialog && <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground z-10 pointer-events-none" />}
      <Input
        ref={inputRef}
        type="text"
