...
export function EpisodeCard({ episode, showId, seasonNumber, onPlay, isPlaying, showBackdropPath }: EpisodeCardProps) {
  ...
  return (
    <div
      className={cn(
        'grid grid-cols-[auto_1fr_auto] items-center gap-4 p-3 group transition-all duration-200 rounded-lg bg-secondary/80',
    ...
    >
      <div
        ...
        className={cn(
            'relative w-32 sm:w-40 flex-shrink-0 aspect-video rounded-md overflow-hidden bg-muted',
            isReleased ? 'cursor-pointer' : 'cursor-default'
        )}
      >
        <Image
